...
The Resources tab uses compact bullet charts for current values and peaks, grouped bar lanes for compute, memory, GPU, I/O, process, and network activity, proportional donut charts for storage/GPU-memory/energy composition, and a combined application-versus-system power area chart. Related measurements share one chart to limit scrolling. Hatched spans represent every unavailable capability or missing sampling period without replacing absent data with text or zero. Hovering the power chart shows values at the nearest real timestamp. Retained history remains available after an application exits.
