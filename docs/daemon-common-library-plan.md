...
All five daemons use the same released common-core and Tokio revision and the shared JSONL runner, including NetworkManager's custom response/event correlation and unit-returning cancellation. The runner provides ordered output, owner-replacement detection, bounded shutdown, and active-ID cleanup consistently. Application, bar, Bluetooth, and clipboard subscriptions use shared owner/destination helpers; their cancellation paths are owner scoped. Bluetooth state persistence is the first secure-state migration. Shelllist's reproducible flake checks its locked daemon revisions, and `tests/check-sibling-boundary.sh` checks candidate sibling worktrees before those locks are advanced.
