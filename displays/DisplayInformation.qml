...
Ui.DetailFlickable {
    ...
    readonly property var statusEntries: availableEntries([
        { label: qsTr("State"), value: output.disabled ? qsTr("Disabled") : qsTr("Enabled"), valueColor: output.disabled ? Ui.Theme.mutedText : Ui.Theme.active, valueBold: !output.disabled },
    ...
    function availableEntries(values: var): var {
        return values.filter(function (entry) { return entry.value !== undefined && entry.value !== null && String(entry.value).length > 0; });
    }

    Repeater {
        model: [
            { name: "displayStatusCard", title: qsTr("Display status"), entries: page.statusEntries },
            { name: "displayIdentityCard", title: qsTr("Display information"), entries: page.identityEntries }
        ]
