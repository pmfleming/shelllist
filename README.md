...
The application surface lists standards-visible desktop applications, groups live Hyprland windows, focuses the most-recent running instance with `Enter`, launches non-running applications, and exposes instances plus desktop-defined actions with `Right`. Running rows use single- or multiple-window icons. Launch-only desktop entries are shown as shortcuts without running/resource state. Application actions have a 20-second frontend recovery watchdog, so a missing terminal lifecycle event refreshes authoritative state instead of leaving the chooser blocked. The details pane separates running instances and desktop-defined actions under **Application** from live usage and compact 30-minute CPU, proportional memory, GPU, disk-I/O, permanent-storage, temporary-storage, and estimated-power graphs under **Resources**. Its header action focuses the first running instance (or launches on the current workspace), **New tile** launches another instance, and each instance has an icon-only focus control with a tooltip. `Shift+Enter` also launches a new tile.
...
The current `WifiProvider.qml`, `BluetoothProvider.qml`, `ClipboardProvider.qml`, and `ApplicationProvider.qml` adapters map their backend protocols onto this model. The application provider consumes `app-api` v1 summaries and routes identifier-only activation, launch, window-focus, and desktop-action requests back to Rust. It keeps the chooser open through accepted/running operation events and closes after asynchronous launch handoff completion; a bounded watchdog restores the chooser if a backend lifecycle is interrupted. Its resource graphs request the latest 30-minute window using the cursor-capable history contract. See [`docs/provider-model.md`](docs/provider-model.md) for the schemas, validation rules, query generations, and action dispatch contract.
