...
The Bluetooth surface provides a keyboard-first **My Devices** list with a separate **Add Device** discovery mode, filtering, global rfkill-aware power plus per-adapter power, bounded discovery with request IDs and cancellation, live and optionally cached signal strength, standard battery display, and contextual pair/connect/disconnect behavior. It shows the cached list immediately, dims last-known signal after discovery ends, and retains recently found devices long enough to pair instead of collapsing to paired devices. `Right` opens a structured details surface with persistent primary/secondary actions and separate Device Details, Audio & Transfer, and Adapter tabs. Device details always place a semantic device visual above the overview, using dedicated artwork for common device classes and an unknown-device fallback, and enrich it with component-battery rings when available, and show the daemon's persisted device icon and last observed battery state while a known device is disconnected, including after reboots; settings include auto-saved rename with original-name reset, trust, wake, block, capability-gated Fast Pair multipoint and noise-control modes, shared destructive confirmation, service and technical summaries; adapter settings include segmented adapter selection, per-adapter power, an auto-saved alias, auto-saved timeout sliders, pairable/discoverable controls, persisted trust-after-pair and reconnect policy, blocked/recent visibility, and enable/disable/restore login behavior. The UI communicates only through `bt-api` v1 and opaque device keys; it does not parse `bluetoothctl` output or route actions on MAC addresses. A `shelllist:bluetooth` Hyprland global shortcut is registered alongside the transition IPC command.
