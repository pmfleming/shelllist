...
Lock & sleep normally occupies one icon row. The information popover explains unavailable actions and separates real sleep blockers from routine preparation handlers, with friendly names and complete reasons. Only blockers, pending operations, service unavailability, or failures add a status row; failed requests offer an explicit Retry. Suspend and Hibernate still require confirmed screen locking. Pending requests disable duplicate actions, and Escape closes the details without dismissing the panel.
