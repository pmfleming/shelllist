...
TestCase {
    ...
    function test_numericBoundsAndLabels(): void {
        ...
        compare(Visuals.collectionMinimum([{ value: -2 }, { value: 5 }, { value: "bad" }], "value"), -2);
        compare(Visuals.collectionMaximum([{ value: -2 }, { value: 5 }, { value: "bad" }], "value"), 5);
    ...
    function test_hourlyForecastAdvancesWithClock(): void {
        ...
        const hours = [0, 1, 2].map(function (offset) {
            return { time_unix_ms: start + offset * 3600000, temperature_c: 10 + offset,
                condition_code: 0, is_day: true };
        });
        const weather = { hourly: hours, utc_offset_seconds: 0 };
        const card = makeCard("WeatherHourlyForecast", { weather: weather, now: new Date(start) });
    ...
    function test_locationCardSupportsKeyboardSelection(): void {
        const card = makeCard("WeatherLocationCard", { modelData: { id: "home", location: "Home", condition_code: 0 }, selectedId: "", now: new Date(), width: 220, height: 92 });
        let selected = "";
        card.selected.connect(function (id) { selected = id; });
    ...
    function test_forecastCardsAcceptEmptyWeather(): void {
        for (const name of ["WeatherHero", "WeatherDailyForecast", "WeatherMetrics"])
            makeCard(name, name === "WeatherHero" ? { weather: {}, now: new Date() } : { weather: {} });
        makeCard("WeatherLocationRail", { locations: [], selectedId: "", now: new Date() });
        wait(10);
    }
}
