...
TestCase {
    ...
    function device(id, health, end) {
        return {
            ...
            energy_now_wh: 30,
            energy_full_wh: 48,
            energy_full_design_wh: 60,
            protection: {
                supported: true,
                enabled: true,
                desired_enabled: true,
                desired_start_percent: end - 5,
                desired_end_percent: end,
                start_percent: end - 5,
                end_percent: end
            }
        };
    ...
    function test_chargeNotificationIsFirstAndTracksChargeTarget() {
        ...
        for (const scenario of [
            {
                ...
                once: false,
                title: "Notify at charge limit (80%)"
            },
            {
                enabled: true,
                end: 85,
                once: false,
                title: "Notify at charge limit (85%)"
            },
            {
                enabled: true,
                end: 85,
            ...
            controller.applyBattery({
                available: true,
                plugged: true,
                devices: [limited, device("BAT1", 90, 85)]
            });
            verify(waitForRendering(panel));
        ...
        toggle.activate();
        verify(!controller.alertDraft.notify_when_full);
