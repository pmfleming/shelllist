...
TestCase {
    ...
    function test_rankingPreservesIdentityAndInputOrder(): void {
        ...
        compare(ranked.map(function (item) {
            return item.id;
        }), ["a", "b"]);
    }

    function test_preparedQueryMatchesIndividualScores(): void {
        const values = [result("cafe", "Café Terminal", 10), result("terminal", "Terminal 9", 5), result("other", "Calculator", 100)];
        for (const query of ["cafe", "TERMINAL 9", "terminal", "missing", ""]) {
            const expected = values.map(function (item) {
                return {
                    item: item,
                    matchScore: Model.matchScore(item, query)
                };
            }).filter(function (entry) {
                return entry.matchScore >= 0;
            }).sort(Model.compareRanked).map(function (entry) {
                return entry.item.id;
            });
            compare(Model.rankResults(values, query).map(function (item) {
                return item.id;
            }), expected);
        }
    }
