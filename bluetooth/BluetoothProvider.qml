...
Core.Provider {
    ...
    function connectionActions(device: var, caps: var): var {
        return [Core.Model.keepOpenAction("pair", "Pair", {
                ...
                confirmation: {
                    required: true,
                    title: "Forget " + (device.name || "Bluetooth device") + "?",
    ...
    function settingActions(device: var, caps: var): var {
        return [trustAction(device, caps), wakeAction(device, caps), multipointAction(device, caps), blockAction(device, caps)];
    }
    ...
    function actionsFor(result: var): var {
        return result && result.payload ? actionsForDevice(result.payload) : [];
    }
    ...
    function execute(request: var): bool {
        return executePayload(request, function (id, payload) {
            return controller.executeDeviceAction(id, payload);
        });
    }
}
