...
ColumnLayout {
    ...
    readonly property var settings: [
        ...
        {
            id: "power_on_connect",
            label: "Power adapter when connecting"
        },
        {
            id: "wait_for_services",
    ...
    readonly property var actions: settings.map(function (setting) {
        return {
            ...
            label: setting.label,
            state: {
                checked: section.policy[setting.id] !== false
            },
            enabled: !section.controller.actionInFlight
        };
    }).concat([
        {
            id: "audio_route_on_connect",
            label: "Use as default output on connect",
            state: {
                checked: policy.audio_route_on_connect === "switch"
            },
    ...
    readonly property var profileOptions: [
        {
            value: "",
            label: "No profile override"
        }
    ].concat(controller.selectedAudioProfiles.map(function (profile) {
        return {
            value: profile.key,
            label: profile.label,
    ...
    Ui.DropDownList {
        ...
        onSelected: function (key) {
            section.controller.updateDevicePolicy({
                preferred_audio_profile_key: key || null
            });
        }
    }
}
