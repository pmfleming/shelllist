...
function declaredClass(value) {
    return String(value.security_class || "").toLowerCase();
}
function securityText(value) {
    return String(value.security || "").toUpperCase();
}
function securityFlags(value) {
    return (Number(value.wpa_flags) || 0) | (Number(value.rsn_flags) || 0);
}
function hasFlag(flags, mask) {
    return (flags & mask) !== 0;
}
function isEnterprise(security, flags) {
    return hasFlag(flags, 0x0200 | 0x2000) || security.includes("ENTERPRISE");
}
function isEnhancedOpen(security, flags) {
    return hasFlag(flags, 0x0800 | 0x1000) || security.includes("OWE");
}
function isPersonal(security, flags) {
    return security.includes("WPA") || hasFlag(flags, 0x0100 | 0x0400);
}
...
function inferredSecurityClass(network) {
    ...
    if (isEnterprise(security, flags))
        return "enterprise";
    if (isEnhancedOpen(security, flags))
        return "enhanced-open";
    if (security === "WEP")
        return "legacy";
    if (isPersonal(security, flags))
        return "personal";
