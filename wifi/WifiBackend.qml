...
Io.DaemonBackend {
    ...
    readonly property var responseHandlerById: ({
            ...
            "hotspot-status": function (value) {
                controller.hotspot.applyStatus(Api.apiData(value, "hotspot") || null);
            },
            "hotspot-start": function (value) {
                controller.hotspot.applyStart(Api.apiResult(value, "result") || ({}));
            },
            "hotspot-stop": function (value) {
                controller.hotspot.applyStop(Api.apiData(value, "result") || ({}));
            },
            "vpn-list": function (value) {
                controller.vpn.applyProfiles(Api.apiData(value, "vpns") || []);
            },
            "vpn-status": function (value) {
                controller.vpn.applyStatus(Api.apiData(value, "vpn") || null);
            },
            "vpn-connect": function (value) {
                controller.vpn.applyConnectStart(Api.apiResult(value, "result") || ({}));
            },
            "vpn-disconnect": function (value) {
                controller.vpn.applyDisconnect(Api.apiData(value, "result") || ({}));
            },
            "inventory": function (value) {
                controller.inventory.applyInventory(Api.apiData(value, "inventory") || null);
            },
            "network-status": function (value) {
                controller.inventory.applyNetworkState(Api.apiData(value, "network") || null);
            },
            "activate-profile": function (value) {
                controller.inventory.applyActivation(Api.apiData(value, "result") || ({}));
            },
            "deactivate-connection": function (value) {
                controller.inventory.applyDeactivation(Api.apiData(value, "result") || ({}));
            },
            "statistics-watch": function (value) {
                controller.statistics.applyStart(Api.apiResult(value, "result") || ({}));
            }
        })
    ...
    function loadBandStatus(path) {
        return call("band-status", NmApi.methods.wifi_band_status, {
            path: path
        });
    }
    function setBand(path, band) {
        return call("band-set", NmApi.methods.wifi_band_set, {
            path: path,
            band: band
        });
    }
    function startScan() {
        return call("scan-start", NmApi.methods.wifi_scan, {
            timeout: 20,
            cache: true
        });
    }
    function setPowered(enabled) {
        return call("power", NmApi.methods.wifi_setEnabled, {
            enabled: enabled
        });
    }
    function connect(request) {
        return call("connect-start", NmApi.methods.wifi_connectTarget, request);
    }
    function disconnect() {
        return call("disconnect", NmApi.methods.wifi_disconnect, {});
    }
    ...
    function loadAdvancedProfile(path) {
        return call("advanced-load", NmApi.methods.wifi_profile_operation, {
            operation: "details",
            path: path
        });
    }
    function saveAdvancedProfile(path, settings) {
        return call("advanced-save", NmApi.methods.wifi_profile_operation, {
            operation: "update",
            path: path,
            settings: settings
        });
    }
    function revealAdvancedSecret(path) {
        return call("advanced-secret", NmApi.methods.wifi_profile_operation, {
            operation: "reveal-secret",
            path: path
        });
    }
    ...
    function cancelSecret(requestId) {
        return call("secret-cancel", NmApi.methods.wifi_secret_provide, {
            request_id: requestId,
            cancel: true
        });
    }
    ...
    function parseQr(payload) {
        return call("qr-parse", NmApi.methods.wifi_qr_parse, {
            payload: payload
        });
    }
    function connectQr(payload, ifname) {
        return call("qr-connect", NmApi.methods.wifi_qr_connect, ifname ? {
            payload: payload,
            ifname: ifname
        } : {
            payload: payload
    ...
    function loadHotspotCapabilities() {
        return call("hotspot-capabilities", NmApi.methods.hotspot_capabilities, {});
    }
    function loadHotspotStatus() {
        return call("hotspot-status", NmApi.methods.hotspot_status, {});
    }
    function startHotspot(options) {
        return call("hotspot-start", NmApi.methods.hotspot_start, options || ({}));
    }
    function stopHotspot() {
        return call("hotspot-stop", NmApi.methods.hotspot_stop, {});
    }

    function loadVpnProfiles() {
        return call("vpn-list", NmApi.methods.vpn_list, {});
    }
    function loadVpnStatus() {
        return call("vpn-status", NmApi.methods.vpn_status, {});
    }
    function connectVpn(request) {
        return call("vpn-connect", NmApi.methods.vpn_connect, request || ({}));
    }
    function disconnectVpn(request) {
        return call("vpn-disconnect", NmApi.methods.vpn_disconnect, request || ({}));
    }

    function loadInventory() {
        return call("inventory", NmApi.methods.network_inventory, {});
    }
    function loadNetworkState() {
        return call("network-status", NmApi.methods.network_status, {});
    }
    function activateProfile(request) {
        return call("activate-profile", NmApi.methods.network_activateProfile, request || ({}));
    }
    function deactivateConnection(request) {
        return call("deactivate-connection", NmApi.methods.network_deactivate, request || ({}));
    }
    function watchStatistics(request) {
        return call("statistics-watch", NmApi.methods.network_statistics_watch, request || ({}));
    }
    ...
    function handleNetworks(envelope) {
        ...
        controller.setBackgroundStatus(networks.length + " " + stale + "cached networks" + (refreshing ? "; refreshing…" : ""));
    }
    ...
    function logForgetResult(result) {
        if (result.operation !== "forget")
            return;
        console.info("shelllist forget result request_id=" + (result.request_id || "") + " ssid=" + (result.ssid || "") + " status=" + (result.status || "unknown") + " disconnected=" + !!result.disconnected + " profiles_found=" + (result.profiles_found || 0) + " profiles_deleted=" + (result.deleted_profiles ? result.deleted_profiles.length : 0) + " profiles_failed=" + (result.failed_profiles ? result.failed_profiles.length : 0));
    }

    function handleProfile(envelope) {
        const result = Api.apiData(envelope, "result") || ({});
        logForgetResult(result);
    ...
    onEventGapDetected: function (stream) {
        controller.handleDaemonEventGap(stream);
    }
