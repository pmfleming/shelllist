...
Rectangle {
    ...
    readonly property real oceanBandWidth: hasOceanBand ? Math.min(720, oceanBandCenter + 15) - oceanBandLeft : 0
    ...
    Accessible.name: "World map of geographic timezone areas. All regions at " + Visuals.utcOffset(offsetSeconds) + " are highlighted, local time " + Visuals.localTime(now.getTime(), offsetSeconds) + (hasCoordinates ? ". Location marked on the map" : "")
    ...
    Item {
        ...
        Rectangle {
            ...
            x: (baseMap.width - baseMap.paintedWidth) / 2 + map.oceanBandLeft / 720 * baseMap.paintedWidth
        ...
        Rectangle {
            ...
            x: (baseMap.width - baseMap.paintedWidth) / 2 + (Math.max(-180, Math.min(180, map.longitude)) + 180) / 360 * baseMap.paintedWidth - width / 2
            y: (baseMap.height - baseMap.paintedHeight) / 2 + (85 - Math.max(-60, Math.min(85, map.latitude))) / 145 * baseMap.paintedHeight - height / 2
            width: 14
            height: 14
            radius: width / 2
