...
function edgeDistance(direction, from, to) {
    if (direction === "l") return from.x - to.x - to.width;
    if (direction === "r") return from.x + from.width - to.x;
    if (direction === "u" || direction === "t") return from.y - to.y - to.height;
...
function directionalMonitor(direction, monitors) {
    const focused = monitors.find(function (monitor) { return monitor.focused; });
    ...
    monitors.forEach(function (monitor) {
        ...
        if (Math.abs(edgeDistance(direction, from, to)) >= 2) return;
...
function relativeMonitor(selector, monitors) {
    const index = monitors.findIndex(function (monitor) { return monitor.focused; });
...
function monitorMatches(selector, monitor, monitors) {
    ...
    if (selector.startsWith("desc:")) return descriptionMatches(selector.slice(5).trim(), monitor);
...
function windowCount(flags, workspace, clients) {
    ...
    return clients.filter(function (client) {
        if (!clientMatches(flags, workspace, client)) return false;
        return !flags.includes("g") || firstInGroup(client, groups);
    }).length;
}

function booleanSelector(value) { return ["true", "1", "yes", "on"].includes(value); }
...
function termMatches(term, workspace, monitor, snapshot) {
    ...
    switch (term[0]) {
    case "r": return rangeMatches(value, workspace);
    case "s": return (workspace.id < -1 && workspace.id > -1337) === booleanSelector(value);
    case "n": return nameMatches(value, workspace);
    case "m": return monitorMatches(value, monitor, snapshot.monitors);
    case "w": return countMatches(value, workspace, snapshot.clients);
    case "f": return fullscreenMatches(value, workspace, snapshot.clients);
    }
    return false;
}
...
function workspaceMatches(selector, workspace, monitor, snapshot) {
    ...
    return terms.every(function (term) { return termMatches(term, workspace, monitor, snapshot); });
}
