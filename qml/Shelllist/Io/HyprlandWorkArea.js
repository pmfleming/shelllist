...
function documentEnd(text, start) {
    ...
    for (let index = start; index < text.length; ++index) {
        ...
        if (advanceString(state, character)) continue;
...
function jsonDocuments(text) {
    ...
    for (let index = 0; index < text.length;) {
        ...
        if (text[index] !== "{" && text[index] !== "[") throw new Error("Invalid Hyprland reply");
...
function monitorBox(monitor) { return WorkspaceRules.monitorBox(monitor); }
function directionalMonitor(direction, monitors) { return WorkspaceRules.directionalMonitor(direction, monitors); }
function monitorMatches(selector, monitor, monitors) { return WorkspaceRules.monitorMatches(selector, monitor, monitors); }
function windowCount(flags, workspace, clients) { return WorkspaceRules.windowCount(flags, workspace, clients); }
function workspaceMatches(selector, workspace, monitor, snapshot) { return WorkspaceRules.workspaceMatches(selector, workspace, monitor, snapshot); }
